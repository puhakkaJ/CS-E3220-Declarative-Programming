...
class DISJ(NaryFormula):
  ...
  def eval(self,KM,w):
    t = self.subformulas[0].eval(KM,w)
    for f in self.subformulas[1:]:
        t = t or f.eval(KM,w)
    return t
...
class NEG:
  ...
  def eval(self,KM,w):
    return not self.subformula.eval(KM,w)
...
class NECE:
  ...
  def eval(self,KM,w):
    for n in KM.neighbors(w):
      if not self.subformula.eval(KM,n):
        return False
    return True
