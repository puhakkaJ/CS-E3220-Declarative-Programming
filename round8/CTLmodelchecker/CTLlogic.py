...
class CTLopEX():
  ...
  def modelCheck(self,bdd,transdata):
    ...
    return BDDEXIST(bdd,newvars,BDDAND(bdd,transbdd,BDDRENAME(bdd,old2new,phibdd)))
...
class CTLopEU():
  ...
  def modelCheck(self,bdd,transdata):
    ...
    # the model-checking of E U.
    transbdd,newvars,oldvars,old2new,new2old = transdata
    phibdd = self.subformula1.modelCheck(bdd,transdata)
    psibdd = self.subformula2.modelCheck(bdd,transdata)
    S = {}
    S[0] = psibdd
    i = 0
    while True:
      S[i+1] = BDDOR(bdd, psibdd, BDDAND(bdd,phibdd,BDDEXIST(bdd,newvars,BDDAND(bdd,transbdd,BDDRENAME(bdd,old2new,S[i])))))
      print("Computation of EG: BDD of size " + str(len(S[i+1])))
      i = i + 1
      if S[i] == S[i-1]:
        print("EG fixpoint at iteration " + str(i) + " for " + str(self))
